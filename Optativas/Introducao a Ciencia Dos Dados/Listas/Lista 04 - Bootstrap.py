...
def CI(data):
    """
    Calcula intervalo de confiança de 99% para a média da nota média das escolas estaduais
    """
    # Filtrar dados para escolas estaduais
    escolas_estaduais = data[data['DEPENDENCIA_ADMINISTRATIVA'] == 'Estadual']

    # Extrair as notas
    notas = escolas_estaduais['NOTA_MEDIA_ESCOLA']

    # Parâmetros para o cálculo
    n = len(notas)
    media = notas.mean()
    desvio_padrao = notas.std(ddof=1)

    # Para 99% de confiança, usamos z = 2.576
    z = ss.norm.ppf(0.99 + (1-0.99)/2)  # 2.576
...
taxa_aprovacao = data['TAXA_DE_APROVACAO'].dropna()
ecdf = ECDF(taxa_aprovacao)

# Plotar
plt.figure(figsize=(10, 6))
plt.plot(ecdf.x, ecdf.y, linewidth=2, color='blue')
plt.xlabel('Taxa de Aprovação (%)', fontsize=12)
plt.ylabel('Probabilidade Acumulada', fontsize=12)
plt.title('Função de Distribuição Acumulada Empírica - Taxa de Aprovação', fontsize=14)
...
def resposta():
    """
    Intervalos de confiança exatos são úteis quando temos pelo menos 50 pontos.
    Grupos com menos de 50 pontos devem usar Bootstrap.

    Baseado na contagem:
    - Federal: 43 pontos (< 50) -> usar Bootstrap
    - Municipal: 22 pontos (< 50) -> usar Bootstrap

    Resposta: 4 (Federal e Municipal)
    """
    return 4

print(f"\nParte A - Grupos que devem usar Bootstrap:")
print(f"Resposta: {resposta()} (Federal e Municipal)")
...
def bootstrap_mean(n_sub, alpha):
    """
    Constrói intervalo de confiança via Bootstrap para a média da NOTA_MEDIA_ESCOLA
    de escolas federais

    Parameters:
    n_sub: número de sub-amostras (5000)
    alpha: nível de confiança (0.9 para 90%)
    """
    # Filtrar dados para escolas federais
    escolas_federais = data[data['DEPENDENCIA_ADMINISTRATIVA'] == 'Federal']
    notas_federais = escolas_federais['NOTA_MEDIA_ESCOLA'].dropna()

    # Array para armazenar as médias das sub-amostras
    values = np.zeros(n_sub)

    # Tamanho da amostra original
    size = len(notas_federais)

    # Bootstrap: amostragem com reposição
    for i in range(n_sub):
        # Amostra com reposição do mesmo tamanho da amostra original
        indices = np.random.choice(len(notas_federais), size=size, replace=True)
        sample = notas_federais.iloc[indices]
        values[i] = sample.mean()

    # Calculando os percentis para o intervalo de confiança
    # Para 90% de confiança: percentis 5% e 95%
    lower_percentile = ((1 - alpha) / 2) * 100  # 5%
    upper_percentile = (alpha + (1 - alpha) / 2) * 100  # 95%

    LI = np.percentile(values, lower_percentile)
    LS = np.percentile(values, upper_percentile)
