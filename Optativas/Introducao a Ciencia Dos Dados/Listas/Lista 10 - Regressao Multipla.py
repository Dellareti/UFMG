...
y = df['preco']
X = df[['area', 'quartos', 'suites', 'vagas']].copy()
X['intercepto'] = 1
...
def descent(theta0, X, y, learning_rate=0.005, tolerance=0.0000001):
  ...
  while True:
    ...
    if np.abs(old_err - err) <= tolerance:
      break
    theta = theta_novo
    old_err = err
  return theta

_teste_param0 = np.array([ 0, 0, 0, 0 ])
theta = descent(_teste_param0, X, y, learning_rate=0.01)
assert_array_almost_equal(theta, np.array([-0.04349112, 0.59055261, -0.23979036, 0.00201339]), decimal=2)
