# -*- coding: utf-8 -*-
"""Lista 09 - Gradient Descent.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1UZKiPz4kxCNVzPX4CKHowOmFx8mq-ahY
...
def z_norm(dataset):
  return (dataset - dataset.mean()) / dataset.std(ddof=1)
...
def descent(x, y, param, lambda_=0.0001, niter=1000):
  ...
  for _ in range(niter):
    ...
    for i in range(len(x)):
      ...
      grad_alpha += da
      grad_beta += db
    alpha = alpha - lambda_ * grad_alpha
    beta = beta - lambda_ * grad_beta
  return [alpha, beta]

[alpha, beta] = descent(df.values[:, 0], df.values[:, 1], [1, 1])

assert_almost_equal(alpha, 0, 1)
assert_almost_equal(beta, 0.982, 2)
...
def descent_vec(x, y, param, lambda_=0.0001, niter=1000):
  alpha, beta = param
  for _ in range(niter):
    residuo = y - beta * x - alpha
    grad_alpha = -2 * np.sum(residuo)
    grad_beta = -2 * np.sum(residuo * x)
    alpha = alpha - lambda_ * grad_alpha
    beta = beta - lambda_ * grad_beta
  return [alpha, beta]
...
def idade_universo(sd_x, sd_y):
  r = np.sqrt(rsquared)
  beta_original = r * sd_y / sd_x
  return (1 / beta_original) / 1000
