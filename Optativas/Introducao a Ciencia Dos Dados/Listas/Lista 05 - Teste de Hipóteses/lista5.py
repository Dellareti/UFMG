...
def percent(df1, df2, alpha):
    """
    Compara se há diferença entre as medianas da variável TAXA_DE_PARTICIPACAO
    entre dois grupos usando bootstrap

    Returns:
    - True se houver diferença
    - False se não houver diferença
    """
    # Número de bootstrap samples
    n_bootstrap = 10000
    ...
    # Array para armazenar as diferenças das medianas
    diff_medians = np.zeros(n_bootstrap)
    ...
    for i in range(n_bootstrap):
        ...
        diff_medians[i] = sample1.median() - sample2.median()

    # Calculando os percentis para o intervalo de confiança
    lower_percentile = (alpha / 2) * 100
    upper_percentile = (1 - alpha / 2) * 100

    LI = np.percentile(diff_medians, lower_percentile)
    LS = np.percentile(diff_medians, upper_percentile)

    # Se o intervalo contém 0, não há diferença significativa
    if LI <= 0 <= LS:
        return False
    else:
        return True
...
def t_obs(data):
    """
    Calcula a estatística de teste (diferença das médias)
    """
    charlotte = data[data['TEAM'] == 'Charlotte Hornets']['SALARY']
    phoenix = data[data['TEAM'] == 'Phoenix Suns']['SALARY']

    # Diferença das médias (Phoenix - Charlotte)
    diff = phoenix.mean() - charlotte.mean()
    return diff
...
def resposta(data):
    """
    Testa se a diferença pode ser explicada pelo acaso usando teste de permutação

    Returns:
    - True se a diferença puder ser explicada pelo acaso
    - False se houver diferença significativa
    """
    ...
    # Array para armazenar as diferenças das permutações
    permuted_diffs = np.zeros(n_permutations)

    # Criando um array com os salários e outro com os times
    salaries = data['SALARY'].values
    teams = data['TEAM'].values.copy()

    for i in range(n_permutations):
        # Embaralhar os times (permutação)
        np.random.shuffle(teams)

        # Calcular diferença das médias para esta permutação
        charlotte_perm = salaries[teams == 'Charlotte Hornets']
        phoenix_perm = salaries[teams == 'Phoenix Suns']

        permuted_diffs[i] = charlotte_perm.mean() - phoenix_perm.mean()

    # Teste bilateral com α = 0.1
    alpha = 0.1

    # Calcular p-valor (teste bilateral)
    p_value = np.sum(np.abs(permuted_diffs) >= np.abs(t_observed)) / n_permutations

    # Se p-valor > α, não rejeitamos H0 (diferença explicada pelo acaso)
...
def t_obs2(data):
    """
    Calcula a estatística de teste para diferença de alturas
    """
    cle_heights = data[data['team_abbreviation'] == 'CLE']['player_height']
    gsw_heights = data[data['team_abbreviation'] == 'GSW']['player_height']

    # Diferença das médias (CLE - GSW)
    diff = cle_heights.mean() - gsw_heights.mean()
    return diff
...
def shuffling(data):
    """
    Realiza o shuffling para o teste de permutação
    """
    N = 5000
    filtro = (data['team_abbreviation'] == 'GSW').values
    diffs = np.zeros(N)
...
def critical_values(diffs):
    """
    Calcula os valores críticos para α = 0.05 (teste bilateral)
    """
    # Para teste bilateral com α = 0.05, usamos 2.5% e 97.5%
...
def p_value(t_obs, diffs):
    """
    Calcula o p-valor para teste unilateral
    """
    return np.mean(diffs <= t_obs)
