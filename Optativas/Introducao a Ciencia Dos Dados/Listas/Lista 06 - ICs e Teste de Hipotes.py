...
(LI, LS) = ic(data)
assert_equal(LI.round(2), 98.08)
assert_equal(LS.round(2), 98.32)
...
def bootstrap_mean(x):
  ...
  LI = np.percentile(bootstrap_means, 2.5)
  LS = np.percentile(bootstrap_means, 97.5)
  return (LI, LS)
...
def t_obs(data):
  ...
  terreas = data[data['floors'] == 1]['price']
  nao_terreas = data[data['floors'] > 1]['price']
  return terreas.mean() - nao_terreas.mean()

tobs = t_obs(data)
assert_equal(round(tobs, 2), -53396.48)
...
def shuffling(data):
  ...
  for _ in range(n_permutations):
    shuffled_prices = data['price'].sample(frac=1).reset_index(drop=True)
    temp_data = data.copy()
    temp_data['price'] = shuffled_prices
    terreas = temp_data[temp_data['floors'] == 1]['price']
    nao_terreas = temp_data[temp_data['floors'] > 1]['price']
    permuted_diffs.append(terreas.mean() - nao_terreas.mean())

  c_inf = np.percentile(permuted_diffs, 2.5)
  c_sup = np.percentile(permuted_diffs, 97.5)
  return (c_inf, c_sup)
