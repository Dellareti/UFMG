...
def mean_error(alpha, beta, x, y):
  return np.mean([abs(error(alpha, beta, x[i], y[i])) for i in range(len(x))])
...
plt.scatter(x, y, alpha=0.8, edgecolors='k', s=80)
plt.plot(x, alpha + beta * x, color='magenta')
plt.xlabel('Altura média dos pais')
plt.ylabel('Altura dos filhos')
plt.title('Regressão linear simples')
despine()
...
def sum_of_squared_errors(alpha, beta, x, y):
  return np.sum([error(alpha, beta, x[i], y[i])**2 for i in range(len(x))])
...
def r_squared(alpha, beta, x, y):
  return 1 - sum_of_squared_errors(alpha, beta, x, y) / np.sum((y - np.mean(y))**2)
