...
unsigned test(RightSquare *s) {
  std::vector<Polygon*> polygons;
  polygons.push_back(new RightSquare(-1, -2, 3));
  polygons.push_back(new Point(2,1));
  polygons.push_back(new RightRectangle(1, 1, 4, 5));
  polygons.push_back(new RightSquare(-2, -2, 4));
...
int main () {
  ...
  switch (type) {
    ...
    case 't':
      std::cout << test(new RightSquare(-2, -2, 4));
      break;
